...
{ struct eventNode *event;
  ...
  { event = eventHead;
    ...
    { case actTQinterrupt:
        ...
        PCB[event->pid]->Qnew = (PCB[event->pid]->Qnum +1);
        //fprintf(stdout,"TQinterrupt-set-incheck timer------ pid=%d,Qnum=%d, Qnew=%d\n",event->pid, PCB[event->pid]->Qnum, PCB[event->pid]->Qnew);
       //  fprintf(stdout,"TimeQuantuminterrupt set  -----------time:%d",CPU.numCycles);
       if(PCB[event->pid]->Qnew >= 4){
         PCB[event->pid]->Qnew = 4;
       } 
        break;
