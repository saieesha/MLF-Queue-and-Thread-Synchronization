...
void init_process_pagetable (int pid, int nop);
...
{ int Pid;
  ...
  int timeUsed;
  int numPF;
  int RQintime; //Ready Queue In Time (Written Manually)
  int Qnum;     // Queue Number storage variable // mod ph2
  int TQused;  // if Time Quantum used up in current queue 
  int TQwaited; // Time Quantum waited in the current queue
  int Qnew;     //Q_to_be_sent_to 
...
int submit_process (char* fname);  // called by submit.c (Written Manually)
  // call loader functions to load the submitted process to swap and memory
  // put the process to ready queue
void execute_process ();  // called by admin.c  (Written Manually)

// increments wait time after clock advances by 1 unit FOR MLFQ IMPLEMENTATION
